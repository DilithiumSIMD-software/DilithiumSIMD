...
static void keccak_squeeze_avx512seq(uint8_t *out,
                                   size_t outlen,
                                   __m512i state[5])
{
  size_t i, j;
...
static void keccak_absorb_once_avx512seq(__m512i state[5],
                               unsigned int r,
                               const uint8_t *in,
                               size_t inlen,
                               uint8_t p)
{
  ...
  while(inlen >= r) {
    for(i = 0; i < 3; i ++)
    {
      state[i] = _mm512_xor_si512(state[i],_mm512_maskz_loadu_epi64(0x1F,(__m512i*)in));
      in += 40;
    }
  ...
  ss[pos] ^= (uint64_t)p << 8*(reslen%8);
  pos = (((r-1)/8)/5)*8+ (((r-1)/8)%5);
  ss[pos] ^= 1ULL << 63;
}
...
static void keccak_squeezeblocks_avx512seq(uint8_t *out,
                                 size_t nblocks,
                                 __m512i state[5])
{
  unsigned int i;
  ...
  while(nblocks) {
    ...
    for(i = 0; i < 3; i ++)
    {
      _mm512_mask_storeu_epi64((__m512*)out,0x1F,state[i]);
      out += 40;
    }
    _mm512_mask_storeu_epi64((__m512*)out,0x03,state[3]);
    out += 16;
    nblocks -= 1;
  }
}
...
void shake256_squeeze_avx512seq(uint8_t *out, size_t outlen, __m512i state[5])
{
  keccak_squeeze_avx512seq(out, outlen, state);
}
/*************************************************
...
void shake256_absorb_once_avx512seq(__m512i state[5], const uint8_t *in, size_t inlen)
{
  keccak_absorb_once_avx512seq(state, SHAKE256_RATE, in, inlen, 0x1F);
}
/*************************************************
...
void shake256_squeezeblocks_avx512seq(uint8_t *out, size_t nblocks, __m512i state[5])
{
  keccak_squeezeblocks_avx512seq(out, nblocks, state);
}
/*************************************************
* Name:        shake128
*
...
//写一个包含init的shake256
void shake256_avx512seq(uint8_t *out, size_t outlen, const uint8_t *in, size_t inlen)
{
  size_t nblocks;
  __m512i state[5];
  // uint64_t state2[25];
