                          ...
                          const uint8_t *sk)
{
  ...
  prepare_s2_table_avx2(s21_table,s22_table, &s2);
  prepare_s1_table_avx2(s11_table,s12_table,&s1);
  /* Compute CRH(tr, msg) */
  shake256_init(&state);
  shake256_absorb(&state, tr, CRHBYTES);
  shake256_absorb(&state, m, mlen);
...
rej:
  ...
  poly_uniform_gamma1_4x(&y.vec[0], &y.vec[1], &y.vec[2], &y.vec[3],
                         rhoprime, nonce, nonce + 1, nonce + 2, nonce + 3);
  poly_uniform_gamma1_4x(&y.vec[4], &y.vec[5], &y.vec[6], &h.vec[0],
                         rhoprime, nonce + 4, nonce + 5, nonce + 6, 0);
  nonce += 7;
#else
#error
  ...
  polyveck w0prime;
  if(evaluate_cs2_earlycheck_avx2(&w0prime, &cp, s21_table,s22_table, &w0, GAMMA2-BETA))
  {
    goto rej;
  }
  if(evaluate_cs1_earlycheck_avx2(&z, &cp,s11_table,s12_table,&y, GAMMA1-BETA))
  {
    goto rej;
  }
  /* Pack z into signature */
