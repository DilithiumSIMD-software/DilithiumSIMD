...
void polyvecl_ntt_lazy_reduction(polyvecl *v) {
  unsigned int i;

  for(i = 0; i < L; ++i)
    poly_ntt_lazy_reduction(&v->vec[i]);
}
void polyveck_ntt_lazy_reduction(polyveck *v) {
  unsigned int i;

  for(i = 0; i < K; ++i)
    poly_ntt_lazy_reduction(&v->vec[i]);
}
void polyveck_ntt_tailored_reduction(polyveck *v) {
  unsigned int i;

  for(i = 0; i < K; ++i)
    poly_ntt_tailored_reduction(&v->vec[i]);
}
void polyvecl_ntt_tailored_reduction(polyvecl *v) {
  unsigned int i;
