                          ...
                          const uint8_t *sk)
{
  ...
  __attribute__((aligned(32))) uint64_t t00_table[4*N],t01_table[4*N];
  ...
  prepare_s1_table_32_avx2(s11_table, s12_table, &s1);
  prepare_s2_table_32_avx2(s21_table, s22_table, &s2);
...
#error
  ...
  polyveck w0prime;
  if(evaluate_cs2_earlycheck_32_avx2(&w0prime, &w0, &cp, s21_table, s22_table, GAMMA2 - BETA))
  {
    goto rej;
  }
  if(evaluate_cs1_earlycheck_32_avx2(&z, &y, &cp, s11_table, s12_table, GAMMA1 - BETA))
  {
    goto rej;
  }
  /* Pack z into signature */
                       ...
                       const uint8_t *pk)
{
  ...
  for(i = 0; i < K; i++) {
    ...
    // poly_shiftl(&t1);
    // poly_ntt(&t1);
    poly_ntt_tailored_reduction(&t1);
