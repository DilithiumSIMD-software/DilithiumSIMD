...
#error
  ...
  /* Transform s1 */
  polyvecl_ntt(&s1);
  // polyvecl_ntt_lazy_reduction(&s1);
...
#error
  ...
  z = y;
  polyvecl_ntt(&y);
  // polyvecl_ntt_tailored_reduction(&y);
                          ...
                          const uint8_t *sk)
{
  ...
  __attribute__((aligned(32))) uint64_t t00_table[2*N],t01_table[2*N];
...
#error
  ...
  polyveck w0prime;
  if(evaluate_cs1_cs2_early_check_32_asm(&z, &w0prime, &cp, s1_table, s2_table, &y, &w0, GAMMA1 - BETA, GAMMA2 - BETA))
  {
    goto rej;
  }
  /* Pack z into signature */
